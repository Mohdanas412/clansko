  ...
  async function handleCreatePost() {
    ...
    try {
      ...
      const res = await fetch('/api/posts/create', {
        ...
        body: JSON.stringify({
          // ✅ FIX: userId removed — the API route now gets the author's identity
          // from the auth session (server-side cookie). Sending it from the client
          // allowed anyone to create posts attributed to any user ID they chose.
  ...
  async function handleReact(postId, type) {
    ...
    try {
      const res = await fetch('/api/posts/react', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          // ✅ FIX: userId removed — the API route now gets the reactor's identity
          // from the auth session. Sending it from the client allowed anyone to
          // react as any other user by passing an arbitrary userId.
          postId,
          type,
        }),
      })
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-8 border-b border-border/60">
        <div>
          <div className="flex items-center gap-2 mb-1.5">
            ...
            <span className="text-xs font-bold tracking-widest text-primary uppercase font-mono">LIVE FEED</span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-foreground">Builder Feed</h1>
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Check out what students are building, give genuine feedback, and team up.
          </p>
        </div>
        <Button onClick={() => setShowModal(true)} size="lg" className="rounded-xl shadow-sm shadow-primary/20 hover:shadow-md hover:shadow-primary/30 h-11 px-5 shrink-0 group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 pt-8">
        ...
        <div className="hidden lg:block lg:col-span-4 space-y-6 shrink-0">
          ...
          <Card className="p-5 rounded-2xl border-border/80 bg-gradient-to-br from-card to-secondary/30 space-y-3 shadow-sm">
            ...
            <Button variant="outline" size="sm" onClick={() => router.push('/goals')} className="w-full text-xs h-9 justify-between rounded-xl bg-background border-border/80">
        ...
        <Modal onClose={() => { setShowModal(false); setFormError(null) }}>
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                placeholder="What are you building? What stack are you using? Need help with anything?"
            ...
            <div className="space-y-1.5">
              <label className="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                Looking For <span className="text-muted-foreground/60 font-normal">(Optional)</span>
              </label>
            ...
            {formError && (
              <div className="p-2.5 rounded-lg bg-red-50 text-red-700 text-xs border border-red-100">{formError}</div>
            )}
 
            <div className="pt-2 border-t border-border/60">
              <Button onClick={handleCreatePost} disabled={formLoading} className="w-full rounded-xl h-11 text-xs font-bold">
      ...
      <div>
        {(teamMembers.length > 0 || isOwner) && (
          <div className="flex items-center justify-between py-3 mb-3 border-y border-border/40 gap-3 flex-wrap bg-secondary/20 -mx-5 px-5 sm:-mx-6 sm:px-6">
            <div className="flex items-center gap-2.5">
              {teamMembers.length > 0 ? (
                <>
                  ...
                  <button onClick={() => router.push(`/projects/${post.id}`)} className="text-[11px] text-primary font-semibold hover:underline">
                    View Project →
                  </button>
                </>
              ) : (
            ...
            {isOwner && (
              <button onClick={() => router.push(`/projects/${post.id}`)} className="text-[11px] font-bold text-primary border border-border/80 bg-background px-2.5 py-1 rounded-lg hover:border-primary/40 transition-colors">
                + Add Team
              </button>
            )}
          </div>
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <button onClick={onExpand} className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-secondary/40 border border-transparent text-xs text-muted-foreground hover:bg-secondary hover:text-foreground transition-all">
...
function Modal({ children, onClose }) {
  return (
    <div onClick={onClose} className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
      <div onClick={e => e.stopPropagation()} className="w-full max-w-lg bg-card border border-border rounded-2xl p-6 shadow-2xl shadow-black/5 max-h-[90vh] overflow-y-auto custom-scrollbar animate-in zoom-in-95 duration-200">
        {children}
      </div>
    </div>
  )
  ...
  async function handleComment() {
    ...
    try {
      ...
      const res = await fetch('/api/posts/comment', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          // ✅ FIX: userId removed — the API route now gets the commenter's
          // identity from the auth session. Sending it from the client allowed
          // anyone to post comments attributed to any other user.
          postId: post.id,
    ...
    <Modal onClose={onClose}>
      <div className="space-y-4">
        <div className="flex items-center justify-between pb-2 border-b border-border/40">
          <span className={cn("px-2.5 py-0.5 rounded-full text-[10px] font-bold border font-mono uppercase", stageObj.colorClass)}>
        ...
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-foreground leading-snug mb-3">{displayPost.title}</h2>
          <div className="flex items-center gap-3 bg-secondary/30 p-2.5 rounded-xl border border-border/40">
            ...
            <div>
              <p className="text-xs font-bold text-foreground leading-none">{displayPost.users?.name || 'Anonymous Peer'}</p>
          ...
          <div className="space-y-1.5 pt-1">
            ...
            <div className="flex flex-wrap gap-1.5">
              {displayPost.looking_for.map(item => (
                <span key={item} className="px-2 py-0.5 rounded bg-primary/5 text-primary border border-primary/10 text-[10px] font-medium font-mono">{item}</span>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="space-y-3 pt-4 border-t border-border/60">
          ...
          {loading && <div className="py-6 text-center text-xs text-muted-foreground animate-pulse">Loading comments...</div>}
          {error && <div className="p-2.5 rounded-lg bg-red-50 text-red-700 text-xs border border-red-100">{error}</div>}
            ...
            <div key={c.id} className="flex gap-2.5 items-start p-3 rounded-xl bg-secondary/30 border border-border/40">
              ...
              <div className="space-y-1 flex-1">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-foreground leading-none">{c.users?.name || 'Student Builder'}</span>
                  <span className="text-[9px] text-muted-foreground font-medium font-mono">{timeAgo(c.created_at)}</span>
                </div>
        ...
        <div className="space-y-2 pt-2">
          ...
          <div className="flex justify-end">
            <Button onClick={handleComment} disabled={commentLoading || !comment.trim()} size="sm" className="rounded-xl px-4 h-9 text-xs font-bold">
