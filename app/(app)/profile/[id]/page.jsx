      ...
      const { data: postsData } = await supabase
        ...
        .select(`
          *,
          ...
          reactions (id, type),
          comments (id, content, created_at, users(name, profile_photo))
        `)
      ...
      <Card className="p-6 sm:p-8 rounded-3xl border-border bg-card shadow-sm relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between relative z-10">
          ...
          <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center min-w-0 flex-1">
            {/* Elegant Modern Multi-Border Avatar Container */}
            ...
            <div className="space-y-2 min-w-0 flex-1">
              ...
              {/* Contextual parameters sequence */}
          ...
          {/* Action Module Matrix Container */}
                  ...
                  <>
                    {pendingConnectionId ? (
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="w-full sm:w-auto rounded-xl text-xs h-9 cursor-not-allowed opacity-50 px-4"
                      >
        ...
        {/* Aggregate Output Stats Strip */}
        <div className="grid grid-cols-3 gap-2 mt-6 pt-4 border-t border-border/60 bg-secondary/10 rounded-2xl p-3 text-center">
          <div>
            <span className="text-base sm:text-lg font-extrabold text-primary block font-mono">
              {userPosts.length}
            </span>
            <span className="text-[10px] text-muted-foreground uppercase tracking-wider block font-mono font-bold">
              Projects
            </span>
          </div>
      ...
      <div className="flex items-center gap-2 border-b border-border/80 pb-px">
        <button
          ...
          className={cn(
            ...
            activeTab === 'posts' 
              ? "text-primary bg-card border-x border-t border-border" 
        ...
        >
          ...
          {activeTab === 'posts' && (
            <span className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-card" />
          )}
        </button>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'posts' ? (
            ...
            >
                ...
                {!profileUser.skills || profileUser.skills.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profileUser.skills.map((skill, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded-md bg-secondary text-foreground text-xs font-medium border border-border/60">
                        {skill}
                      </span>
                    ))}
                  </div>
              ...
              {/* Collaboration Intent Tags block */}
              <Card className="p-5 rounded-2xl border-border bg-card space-y-3">
                <span className="text-[10px] font-bold text-primary uppercase tracking-wider block font-mono">
                  Looking For
                </span>
                ...
                {!profileUser.looking_for || profileUser.looking_for.length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {profileUser.looking_for.map((item, idx) => (
                      <span key={idx} className="px-2.5 py-1 rounded-md bg-primary/5 text-primary text-xs font-medium border border-primary/20">
                        {item}
                      </span>
                    ))}
                  </div>
...
// ── COMPONENTIZED POST FEED STREAM MODULE ────────────────────────────────────
  ...
  return (
    <Card className="p-5 sm:p-6 rounded-2xl border-border bg-card shadow-xs hover:shadow-sm transition-all duration-200 space-y-4">
      ...
      <div className="flex items-center justify-between gap-3 pb-3 border-b border-border/60">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <span className="text-xs sm:text-sm font-bold text-foreground truncate block">
              {post.users?.name || 'Student Builder'}
            </span>
        ...
        {post.stage && (
          <span className="px-2.5 py-0.5 rounded-md bg-primary/10 text-primary text-[10px] font-bold font-mono tracking-wide uppercase shrink-0">
            {post.stage}
          </span>
        )}
      </div>
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed font-normal whitespace-pre-wrap">
          {post.description}
        </p>
      </div>
      ...
      {post.looking_for && post.looking_for.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 pt-1">
          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider font-mono mr-1">
            Looking for:
          </span>
          {post.looking_for.map((item, idx) => (
            <span key={idx} className="px-2 py-0.5 rounded bg-secondary text-foreground text-[10px] font-medium border border-border/60">
              {item}
            </span>
          ))}
        </div>
      ...
      {/* Nested Expandable Comments canvas block */}
      {showComments && (
        <div className="pt-3 border-t border-border/60 space-y-3 animate-in fade-in duration-200">
          <form onSubmit={handleComment} className="flex items-center gap-2">
            ...
            <Button
              ...
              className="h-8 px-3 rounded-xl text-xs shrink-0 shadow-xs"
            >
                ...
                <div key={c.id} className="p-2.5 rounded-xl bg-background border border-border/60 space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-xs font-bold text-foreground truncate">
                      {c.users?.name || 'Anonymous Student'}
                    </span>
